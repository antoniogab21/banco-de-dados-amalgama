...
import { Plus, MessageSquare, Users, Package, TrendingUp } from 'lucide-react';
...
export default function SocialFeed({ userType, onNavigate }: SocialFeedProps) {
  const { orders, createOrder, joinOrder } = useApp();
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-2">Feed de Grupos</h1>
              <p className="text-gray-600">Acompanhe pedidos coletivos e participe</p>
            </div>
            {userType === 'mercado' && (
              <motion.button
              ...
              >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4 text-sm opacity-90 mt-1">
                          ...
                          <span>•</span>
                          <span>{groupOrders.length} pedidos ativos</span>
                        </div>
                      </div>
                    </div>
                    <motion.button
                ...
                <div className="p-6 space-y-4">
                  {groupOrders.map((order) => {
                    const isParticipating = order.participants.some(p => p.company === 'Minha Empresa');
                    const pricePerUnit = 22.90;
                    const totalValue = order.quantity * pricePerUnit;
                    ...
                    >
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">Criado por {order.creator}</p>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-4 mb-4">
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <p className="font-bold text-gray-900">{order.participants.length}</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3">
                          <p className="text-xs text-gray-500 mb-1">Quantidade Atual</p>
                          <p className="font-bold text-gray-900">{order.quantity}</p>
                        </div>
                        <div className="bg-yellow-50 rounded-lg p-3">
                          <p className="text-xs text-yellow-700 mb-1">Valor Total</p>
                          <p className="font-bold text-yellow-700">R$ {totalValue.toFixed(2)}</p>
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs text-gray-500">Progresso do pedido</span>
                        ...
                        <div className="w-full bg-gray-100 rounded-full h-2">
                          <div
                            ...
                            style={{ width: `${(order.participants.length / group.members) * 100}%` }}
                          />
                        </div>
                      </div>
                      ...
                      {order.participants.length > 0 && (
                        <div className="bg-gray-50 rounded-xl p-4 mb-4">
                          <p className="text-xs font-bold text-gray-500 uppercase mb-3">Participantes:</p>
                          <div className="space-y-2">
                            {order.participants.map((participant, idx) => (
                              <div key={idx} className="flex items-center justify-between">
                                <span className="text-sm text-gray-700">{participant.company}</span>
                      ...
                      {userType === 'mercado' && !isParticipating && (
